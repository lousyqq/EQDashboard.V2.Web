...
- **架構**：Kestrel/IIS；後端 Service 層 + DI 解耦；前端 ES Modules + Bootstrap 5/Vanilla JS（無 npm/bundler；第三方資產自 2026-08-15 起**全部自 host 於 `wwwroot/lib`**，已不走 CDN）。
...
- **CSP/SRI**：2026-08-15 起第三方資產（Bootstrap / FontAwesome / jQuery / DataTables / SheetJS）**全部自 host 於 `wwwroot/lib`**，CSP 原本的四個外部 CDN 來源（`cdn.jsdelivr.net`/`cdnjs.cloudflare.com`/`cdn.datatables.net`/`code.jquery.com`）已一併移除，`script-src`/`style-src` 收斂為 `self` + `unsafe-inline`、`font-src` 為 `self` + `data:`。**嚴禁改回 CDN URL，也不需要 `integrity`/`crossorigin`**（SRI 是防第三方主機被竄改，同源檔案不適用，保留反而會在檔案有任何位元差異時整支資產被擋掉）。`frame-src`/`img-src` 仍放行 http:/https: 供外部看板 iframe 與圖示。現行 CSP 會直接擋下任何改回 CDN 的嘗試（F12 Console 有明確違規訊息）——這是刻意的，請改回自 host 而非放寬政策。
...
- [x] **DataProtection 金鑰輪換（安全優先）**：已於 2026-08-24 清除歷史外洩的 App_Data/keys/*。
